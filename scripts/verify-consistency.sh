...
# Check 8: KV namespace IDs (no placeholders)
echo "📋 [8/9] Checking for KV namespace placeholder values..."
if grep -E "YOUR_.*_ID|<.*>" wrangler.jsonc 2>/dev/null; then
  echo "❌ ERROR: Found placeholder values in wrangler.jsonc"
  echo "   Fix: Replace ALL placeholders with actual IDs from CLOUDFLARE_CONFIG.md"
  ERRORS=$((ERRORS + 1))
else
  echo "✅ No placeholder values in wrangler.jsonc"
fi
echo ""
